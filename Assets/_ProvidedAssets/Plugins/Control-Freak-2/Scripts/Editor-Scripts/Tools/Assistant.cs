...
namespace ControlFreak2Editor
{
    public class Assistant : EditorWindow
    {
        ...
        // ---------------------
        static public void CaptureKey(KeyCode keyParam)
        {
            CaptureEx(CallType.Key, "", keyParam);
        }

        static public void CaptureButton(string buttonName)
        {
            CaptureEx(CallType.Button, buttonName, KeyCode.None);
        }

        static public void CaptureAxis(string axisName)
        {
            CaptureEx(CallType.Axis, axisName, KeyCode.None);
        }

        static public void CaptureMousePos()
        {
            CaptureEx(CallType.MousePosition, "", KeyCode.None);
        }

        static public void CaptureScrollWheel()
        {
            CaptureEx(CallType.ScrollWheel, "", KeyCode.None);
        }

        static public void CaptureTouch()
        {
            CaptureEx(CallType.Touch, "", KeyCode.None);
        }

        static public void CaptureCursorLock()
        {
            CaptureEx(CallType.CursorLock, "", KeyCode.None);
        }
        ...
        void OnGUI()
        {
            ...
            this.recordingOn = CFGUI.PushButton(
                new GUIContent("Capture in progress!", CFEditorStyles.Inst.greenDotTex,
                    "Editor must be in Play mode to catch Input calls!"),
                new GUIContent("Stopped. Press to begin capture", CFEditorStyles.Inst.pauseTex,
                    "Editor must be in Play mode to catch Input calls!"),
                this.recordingOn, CFEditorStyles.Inst.buttonStyle, GUILayout.Height(BUTTON_HEIGHT),
            ...
            if (this.selectedEntry != null)
            {
                ...
                if (this.propBoxOn)
                {
                    this.propBoxScroll = EditorGUILayout.BeginScrollView(this.propBoxScroll,
                        CFEditorStyles.Inst.transpSunkenBG, GUILayout.Height(propBoxHeight));
        ...
        public class LogEntry
        {
            ...
            {
                return ((this.callType == call) && (this.strParam == strParam) && (this.keyParam == keyParam) &&
            ...
            public void DrawListElemGUI(Assistant listener)
            {
                ...
                switch (this.callType)
                {
                    case CallType.Axis:
                    case CallType.Button:
                        nameContent = new GUIContent(this.strParam,
                            ((this.callType == Assistant.CallType.Axis) ? "Axis Name" : "Button Name"));

                        if (listener.selectedRig != null)
                        {
                            if (!this.availableInRig)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texError,
                                    "Axis not available in [" + listener.selectedRig.name + "] rig!");

                            else if (!this.availableOnMobile)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texWarning,
                                    "Axis not available in [" + listener.selectedRig.name + "] rig in Mobile Mode!");

                            else
                                stateContent = new GUIContent(CFEditorStyles.Inst.texOk,
                                    "Axis available in desktop and mobile modes!");
                        }
                    ...
                    case CallType.Key:
                        ...
                        if (listener.selectedRig != null)
                        {
                            if (!this.availableInRig)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texError,
                                    "Key not available in [" + listener.selectedRig.name + "] rig!");

                            else if (!this.availableOnMobile)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texWarning,
                                    "Key not available in [" + listener.selectedRig.name + "] rig in Mobile Mode!");

                            else
                                stateContent = new GUIContent(CFEditorStyles.Inst.texOk,
                                    "Key available in desktop and mobile modes!");
                        }
                    ...
                    case CallType.ScrollWheel:
                        if (listener.selectedRig != null)
                        {
                            if (!this.availableInRig)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texError,
                                    "Scroll Wheel not available in [" + listener.selectedRig.name +
                                    "] rig! Create an axis of SCROLL type and bind it as one of scroll wheel axes in Input Rig's Scroll Wheel Settings!");

                            else if (!this.availableOnMobile)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texWarning,
                                    "Scroll Wheel not available in [" + listener.selectedRig.name +
                                    "] rig in Mobile Mode! Create an axis of SCROLL type and bind it as one of scroll wheel axes in Input Rig's Scroll Wheel Settings!");

                            else
                                stateContent = new GUIContent(CFEditorStyles.Inst.texOk,
                                    "Scroll Wheel available in desktop and mobile modes!");
                        }
                    ...
                    case CallType.MousePosition:
                        if (listener.selectedRig != null)
                        {
                            if (!this.availableInRig)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texError,
                                    "Mouse Position is not available in [" + listener.selectedRig.name +
                                    "] rig! Create an Touch Zone and bind mouse position to it.");
                                ...
                                stateContent = new GUIContent(CFEditorStyles.Inst.texOk,
                                    "Mouse Position available in desktop and mobile modes!");
                        }
                    ...
                    case CallType.Touch:
                        if (listener.selectedRig != null)
                        {
                            if (!this.availableInRig)
                                stateContent = new GUIContent(CFEditorStyles.Inst.texError,
                                    "Emulated Touches are not available in [" + listener.selectedRig.name +
                                    "] rig! Create an Touch Zone and bind emulated touches to it.");
                                ...
                                stateContent = new GUIContent(CFEditorStyles.Inst.texOk,
                                    "Emulated Touches are available in desktop and mobile modes!");
                        }
                ...
                if ((listener.selectedRig !=
                     null)) //&& ((this.callType == Assistant.CallType.Axis) || (this.callType == CallType.Button)) && !this.availableInRig)
                {
                    switch (this.callType)
                    {
                        case CallType.Axis:
                            if (GUILayout.Button(new GUIContent(
                                    (this.availableInRig
                                        ? CFEditorStyles.Inst.texWrench
                                        : CFEditorStyles.Inst.createNewTex),
                                    "Add or bind this axis..."), GUILayout.Width(20), GUILayout.Height(HEIGHT)))
                            {
                                WizardMenuUtils.CreateContextMenuForAxisBinding(listener.selectedRig, this.strParam,
                                    listener.RefreshRigState);
                            }
                        ...
                        case CallType.Key:
                            ...
                            {
                                WizardMenuUtils.CreateContextMenuForKeyBinding(listener.selectedRig, this.keyParam,
                                    listener.RefreshRigState);
                            }
                        ...
                        case CallType.MousePosition:
                            ...
                            {
                                WizardMenuUtils.CreateContextMenuForMousePositionBinding(listener.selectedRig,
                                    listener.RefreshRigState);
                            }
                        ...
                        case CallType.Touch:
                            ...
                            {
                                WizardMenuUtils.CreateContextMenuForEmuTouchBinding(listener.selectedRig,
                                    listener.RefreshRigState);
                            }
                ...
                if (GUILayout.Button(new GUIContent(this.callType.ToString()), CFEditorStyles.Inst.transpBevelBG,
                        GUILayout.Width(100), GUILayout.Height(HEIGHT)))
                    showPropertiesClicked = true;

                GUI.backgroundColor = new Color(1, 1, 1, 0.3f);
                if (GUILayout.Button(nameContent, CFEditorStyles.Inst.whiteBevelBG, GUILayout.ExpandWidth(true),
            ...
            public void DrawPropertiesGUI()
            {
                ...
                for (int i = 0; i < this.frames.Count; ++i)
                {
                    ...
                    if (GUILayout.Button(
                            new GUIContent( //"Method: " + fr.method + "\n" + "Class: + " + fr.classname + "\n" +
                                " Line: " + fr.line + " in " + fr.relFilename, CFEditorStyles.Inst.magnifiyingGlassTex,
                                "Open this line in IDE.")))
                    {
                        fr.ShowInIDE();
                    }
                }

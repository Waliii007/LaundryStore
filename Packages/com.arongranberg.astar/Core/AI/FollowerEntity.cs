...
namespace Pathfinding
{
    ...
    public sealed partial class FollowerEntity : VersionedMonoBehaviour, IAstarAI, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] ManagedState managedState = new ManagedState
        {
            enableLocalAvoidance = false,
            pathfindingSettings = PathRequestSettings.Default,
        };

        [SerializeField] ECS.AutoRepathPolicy autoRepathBacking = ECS.AutoRepathPolicy.Default;
        ...
        /// </summary>
        [SerializeField] OrientationMode orientationBacking;

        [SerializeField] MovementPlaneSource movementPlaneSourceBacking = MovementPlaneSource.Graph;
        ...
        static EntityAccess<MovementStatistics>
            movementStatisticsAccessRW = new EntityAccess<MovementStatistics>(false);
        ...
        static EntityAccess<AgentCylinderShape>
            agentCylinderShapeAccessRW = new EntityAccess<AgentCylinderShape>(false);
        ...
        static EntityAccess<AgentOffMeshLinkTraversal> agentOffMeshLinkTraversalRO =
            new EntityAccess<AgentOffMeshLinkTraversal>(true);

        static EntityAccess<ReadyToTraverseOffMeshLink> readyToTraverseOffMeshLinkRW =
            new EntityAccess<ReadyToTraverseOffMeshLink>(false);
        ...
        /// </summary>
        public static Entity CreateEntity(float3 position, quaternion rotation, float scale,
            ref AgentCylinderShape shape, ref MovementSettings movement, ref ECS.AutoRepathPolicy autoRepath,
            ManagedState managedState, OrientationMode orientation, MovementPlaneSource movementPlaneSource,
            bool updatePosition, bool updateRotation, PhysicsScene physicsScene)
        {
            var world = World.DefaultGameObjectInjectionWorld;
            if (!archetype.Valid || achetypeWorld != world)
            {
                if (world == null)
                    throw new Exception("World.DefaultGameObjectInjectionWorld is null. Has the world been destroyed?");
                achetypeWorld = world;
            ...
            if (!managedState.pathTracer.hasPath && AstarPath.active != null)
            {
                ...
                if (nearest.node != null)
                {
                    ...
                    if (movementPlaneSource == MovementPlaneSource.Graph)
                    {
                        ...
                        var targetRotationHint =
                            movementPlane.value.ToWorldRotation(resolvedMovement.targetRotationHint);
                        var targetRotation2 = movementPlane.value.ToWorldRotation(movementControl.targetRotation);
                        var targetRotationHint2 =
                            movementPlane.value.ToWorldRotation(movementControl.targetRotationHint);

                        movementPlane =
                            new AgentMovementPlane(
                                MovementPlaneFromGraphSystem.MovementPlaneFromGraph(nearest.node.Graph));
                    ...
                    managedState.pathTracer.SetFromSingleNode(nearest.node, nearest.position, movementPlane.value,
                        managedState.pathfindingSettings);
                    // Make the end of the path be unset
                    managedState.pathTracer.UpdateEnd(Vector3.positiveInfinity, PathTracer.RepairQuality.High,
                        movementPlane.value, null, null);
                }
            }
        }
                        ...
                        out var storage))
                {
                    ...
                    if (managedState.pathTracer.hasPath)
                    {
                        ...
                        managedState.pathTracer.UpdateStart(value, PathTracer.RepairQuality.High, movementPlane.value,
                            managedState.pathfindingSettings.traversalProvider, managedState.activePath);
                        Profiler.EndSample();
                        if (managedState.pathTracer.version != oldVersion)
                        {
                            ...
                            var readyToTraverseOffMeshLink = storage.Chunk
                                .GetEnabledMask(ref readyToTraverseOffMeshLinkRW.handle)
                                .GetEnabledRefRW<ReadyToTraverseOffMeshLink>(storage.IndexInChunk);
                            if (!nextCornersScratch.IsCreated)
                                nextCornersScratch = new NativeList<float3>(4, Allocator.Persistent);
        ...
        public OffMeshLinks.OffMeshLinkTracer offMeshLink
        {
            ...
            {
                if (entityStorageCache.Update(World.DefaultGameObjectInjectionWorld, entity, out var entityManager,
                        out var storage) && entityManager.HasComponent<AgentOffMeshLinkTraversal>(entity))
                {
                    ...
                    return new OffMeshLinks.OffMeshLinkTracer(linkTraversalManaged.context.concreteLink,
                        linkTraversal.relativeStart, linkTraversal.relativeEnd, linkTraversal.isReverse);
                }
                else
                {
                    return default;
        ...
        public Quaternion rotation
        {
            ...
            set
            {
                        ...
                        out var storage))
                {
                    ...
                    if (orientation == OrientationMode.YAxisForward)
                        value = math.mul(value, SyncTransformsToEntitiesSystem.YAxisForwardToZAxisForward);
                    localTransformAccessRW.Update(entityManager);
                ...
                else
                {
                    ...
                    {
                        Debug.LogWarning(
                            "Cannot set agent rotation because updateRotation is false and the FollowerEntity component has not been enabled yet. Therefore, the internal entity does not exist, and there's no rotation to set.",
                            this);
                    }
                }
            }
        ...
        public Vector3 velocity
        {
            ...
            {
                return entityExists
                    ? (Vector3)World.DefaultGameObjectInjectionWorld.EntityManager
                        .GetComponentData<MovementStatistics>(entity).estimatedVelocity
                    : Vector3.zero;
            }
            set
            {
        ...
        public Vector3 desiredVelocity
        {
            get
            {
                if (entityStorageCache.GetComponentData(entity, ref resolvedMovementAccessRO, out var resolvedMovement))
                {
                    ...
                    return Vector3.ClampMagnitude((Vector3)resolvedMovement.value.targetPoint - position,
                        dt * resolvedMovement.value.speed) / dt;
                }
                else
                {
                    return Vector3.zero;
        ...
        public Vector3 desiredVelocityWithoutLocalAvoidance
        {
            get
            {
                ...
                {
                    var dt = Mathf.Max(Time.deltaTime, 0.0001f);
                    return Vector3.ClampMagnitude((Vector3)movementControl.value.targetPoint - position,
                        dt * movementControl.value.speed) / dt;
                }
                else
                {
                    return Vector3.zero;
            ...
            set => throw new NotImplementedException(
                "The FollowerEntity does not support setting this property. If you want to override the movement, you'll need to write a custom entity component system.");
        }
        ...
        public float remainingDistance
        {
            ...
            {
                if (!entityStorageCache.Update(World.DefaultGameObjectInjectionWorld, entity, out var entityManager,
                        out var storage)) return float.PositiveInfinity;
                ...
                if (managedState.pathTracer.hasPath && !managedState.pathTracer.isStale)
                {
                    ...
                    return movementState.remainingDistanceToEndOfPart +
                           Vector3.Distance(managedState.pathTracer.endPointOfFirstPart,
                               managedState.pathTracer.endPoint);
                }
                else
                {
        ...
        public bool reachedDestination =>
            entityStorageCache.GetComponentData(entity, ref movementStateAccessRW, out var movementState)
                ? movementState.value.reachedDestinationAndOrientation
                : false;
        ...
        public bool reachedEndOfPath =>
            entityStorageCache.GetComponentData(entity, ref movementStateAccessRW, out var movementState)
                ? movementState.value.reachedEndOfPathAndOrientation
                : false;
        ...
        {
            get => entityStorageCache.GetComponentData(entity, ref destinationPointAccessRO, out var destination)
        ...
        {
            get => entityStorageCache.GetComponentData(entity, ref destinationPointAccessRO, out var destination)
                ? (Vector3)destination.value.facingDirection
                : Vector3.zero;
        }
        ...
        {
            get { return autoRepathBacking.mode != AutoRepathPolicy.Mode.Never; }
            set
            {
                if (value)
                {
        ...
        public NativeMovementPlane movementPlane =>
            entityStorageCache.GetComponentData(entity, ref movementPlaneAccessRO, out var movementPlane)
                ? movementPlane.value.value
                : new NativeMovementPlane(rotation);
            ...
            if (world == null || !world.EntityManager.Exists(entity))
            {
                if (!mustExist)
                    throw new System.InvalidOperationException(
                        "Entity does not exist. You can only access this if the component is active and enabled.");
                return;
            }
        ...
        static void ToggleComponentEnabled<T>(Entity entity, bool enabled, bool mustExist)
        ...
        public Vector3 steeringTarget =>
            entityStorageCache.GetComponentData(entity, ref movementStateAccessRO, out var movementState)
                ? (Vector3)movementState.value.nextCorner
                : position;
        ...
        /// </summary>
        public ManagedMovementOverrides movementOverrides =>
            new ManagedMovementOverrides(entity, World.DefaultGameObjectInjectionWorld);
        ...
        void IAstarAI.FinalizeMovement(Vector3 nextPosition, Quaternion nextRotation)
        {
            throw new InvalidOperationException(
                "The FollowerEntity component does not support FinalizeMovement. Use an ECS system to override movement instead, or use the movementOverrides property. If you just want to move the agent to a position, set ai.position or call ai.Teleport.");
        }
        ...
        public void GetRemainingPath(List<Vector3> buffer, List<PathPartWithLinkInfo> partsBuffer, out bool stale)
        {
            ...
            if (ms.pathTracer.hasPath)
            {
                ...
                ms.pathTracer.GetNextCorners(nativeBuffer, int.MaxValue, ref scratch, Allocator.Temp,
                    ms.pathfindingSettings.traversalProvider, ms.activePath);
                    ...
                    while (pathTracer.partCount > 1)
                    {
                        ...
                        if (pathTracer.GetPartType() == Funnel.PartType.NodeSequence)
                        {
                            pathTracer.GetNextCorners(nativeBuffer, int.MaxValue, ref scratch, Allocator.Temp,
                                ms.pathfindingSettings.traversalProvider, ms.activePath);
                            if (partsBuffer != null)
                                partsBuffer.Add(new PathPartWithLinkInfo(startIndex, nativeBuffer.Length - 1));
                        }
                        else
                        {
                            ...
                            if (partsBuffer != null)
                                partsBuffer.Add(new PathPartWithLinkInfo(startIndex - 1, startIndex,
                                    pathTracer.GetLinkInfo()));
                        }
        ...
        /// See: <see cref="entity"/>
        /// </summary>
        public bool entityExists => World.DefaultGameObjectInjectionWorld != null &&
                                    World.DefaultGameObjectInjectionWorld.EntityManager.Exists(entity);
        ...
        static void ClearPath(Entity entity)
        {
                    ...
                    out var storage))
            {
                ...
                if (agentOffMeshLinkTraversalRO.HasComponent(storage))
                {
                    ...
                    entityStorageCache.Update(World.DefaultGameObjectInjectionWorld, entity, out entityManager,
                        out storage);
                }
                ...
                ResetControl(ref resolvedMovement, ref controlOutput, ref movementPlane, localTransform.Position,
                    localTransform.Rotation, movementState.endOfPath);
            }
        }
        ...
        public void SetPath(Path path, bool updateDestinationFromPath = true) =>
            SetPath(entity, path, updateDestinationFromPath);
        ...
        public static void SetPath(Entity entity, Path path, bool updateDestinationFromPath = true)
        {
            ...
            if (!entityManager.Exists(entity))
                throw new System.InvalidOperationException(
                    "Entity does not exist. You can only assign a path if the component is active and enabled.");
            ...
            if (updateDestinationFromPath && path is ABPath abPath)
            {
                ...
                if (abPath.endPointKnownBeforeCalculation)
                {
                    destination = new DestinationPoint
                        { destination = abPath.originalEndPoint, facingDirection = default };
                }
                else
                {
                    ...
                    destination = new DestinationPoint
                        { destination = Vector3.positiveInfinity, facingDirection = default };
                }
            }
            ...
            autoRepathPolicy.DidRecalculatePath(destination.destination,
                (float)World.DefaultGameObjectInjectionWorld.Time.ElapsedTime);
            ...
            if (path.IsDone())
            {
                ...
                managedState.pathTracer.UpdateStart(localTransform.Position, PathTracer.RepairQuality.High,
                    movementPlane.value, managedState.pathfindingSettings.traversalProvider, managedState.activePath);
                managedState.pathTracer.UpdateEnd(destination.destination, PathTracer.RepairQuality.High,
                    movementPlane.value, managedState.pathfindingSettings.traversalProvider, managedState.activePath);
        ...
        protected override void OnUpgradeSerializedData(ref Serialization.Migrations migrations, bool unityThread)
        {
            if (migrations.TryMigrateFromLegacyFormat(out var _legacyVersion))
            {
                ...
                if (this.pathfindingSettings.tagPenalties.Length != 0)
                    migrations.MarkMigrationFinished((int)FollowerEntityMigrations.MigratePathfindingSettings);
            }

            if (migrations.AddAndMaybeRunMigration((int)FollowerEntityMigrations.MigratePathfindingSettings,
                    unityThread))
            {
        ...
        public void SyncWithEntity()
        {
            ...
            SyncWithEntity(managedStateAccessRW[storage], ref agentCylinderShapeAccessRW[storage],
                ref movementSettingsAccessRW[storage]);
        ...
        /// Note: This is an internal method and you should never need to use it yourself.
        /// </summary>
        public void SyncWithEntity(ManagedState managedState, ref AgentCylinderShape shape,
